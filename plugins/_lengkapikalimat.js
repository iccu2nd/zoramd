...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/lengkapikalimat.json'
...
function makeClue(word) {
  return word.replace(/[AIUEOaiueo]/g, '_')
}

function endGame(sock, chat) {
  const game = sock.lengkapikalimat?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.lengkapikalimat[chat]
}

export default {
  cmd: ['lengkapikalimat', 'lengkapi'],
  category: 'games',

  run: async (m, { sock, prefix, cmd }) => {
    sock.lengkapikalimat ??= {}

    if (cmd === 'lengkapi') {
      const game = sock.lengkapikalimat[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}lengkapikalimat*.`)
      ...
      const clue = makeClue(game.soal.jawaban.trim())
    ...
    const reward = randRange(REWARD_MIN, REWARD_MAX)
...
Ketik *${prefix}lengkapi* untuk melihat petunjuk.
Hadiah: ${reward} Money`.trim()
    ...
    const timer = setTimeout(() => {
      if (sock.lengkapikalimat?.[m.chat]) {
        sock.sendMessage(m.chat, { text: `Waktu habis. Jawabannya adalah *${soal.jawaban.trim()}*.` }, { quoted: sent }).catch(() => {})
        delete sock.lengkapikalimat[m.chat]
      }
    }, TIMEOUT_MS)

    sock.lengkapikalimat[m.chat] = { key: sent.key, soal, reward, timer }
  },
  ...
  onMessage: async (m, { sock }) => {
    ...
    if (surrenderRegex.test(guess)) {
      endGame(sock, m.chat)
      m.reply(`Baik, permainan diakhiri. Jawabannya adalah *${game.soal.jawaban.trim()}*.`)
      return true
    }
