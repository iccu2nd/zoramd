...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/tebaklogo.json'
...
function endGame(sock, chat) {
  const game = sock.tebaklogo?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.tebaklogo[chat]
}
...
export default {
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'tlogo') {
      const game = sock.tebaklogo[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}tebaklogo*.`)
    ...
    const caption = `*TEBAK LOGO*

${soal.deskripsi}

Waktu: ${(TIMEOUT_MS / 1000).toFixed(0)} detik. Balas pesan ini untuk menjawab.
Ketik *${prefix}tlogo* untuk melihat petunjuk.
Hadiah: ${reward} Money`.trim()

    const sent = await sock.sendMessage(m.chat, { image: { url: soal.img }, caption }, { quoted: m })

    const timer = setTimeout(() => {
      if (sock.tebaklogo?.[m.chat]) {
        sock.sendMessage(m.chat, { text: `Waktu habis. Jawabannya adalah *${soal.jawaban}*.` }, { quoted: sent }).catch(() => {})
        delete sock.tebaklogo[m.chat]
      }
    }, TIMEOUT_MS)

    sock.tebaklogo[m.chat] = { key: sent.key, soal, reward, timer }
  },
