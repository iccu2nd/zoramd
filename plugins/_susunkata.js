...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/susunkata.json'
...
function makeClue(word) {
  ...
  return letters
    .map((ch, i) => (i === 0 || i === letters.length - 1 || i % 2 === 0 ? ch : '_'))
    .join(' ')
}

function endGame(sock, chat) {
  const game = sock.susunkata?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.susunkata[chat]
}
...
export default {
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'skata') {
      const game = sock.susunkata[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}susunkata*.`)
      ...
      return m.reply(`Tipe: ${game.soal.tipe}\nPetunjuk: ` + '```' + clue + '```' + note + '\n\nBalas pesan soal untuk menjawab, bukan pesan ini.')
    }

    if (sock.susunkata[m.chat]) {
...
Ketik *${prefix}skata* untuk melihat petunjuk.
Hadiah: ${reward} Money`.trim()
    ...
    const timer = setTimeout(() => {
      if (sock.susunkata?.[m.chat]) {
        ...
        delete sock.susunkata[m.chat]
      }
    }, TIMEOUT_MS)

    sock.susunkata[m.chat] = { key: sent.key, soal, reward, timer }
  },
