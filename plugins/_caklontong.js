...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/caklontong.json'
...
function endGame(sock, chat) {
  const game = sock.caklontong?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.caklontong[chat]
}

export default {
  cmd: ['caklontong', 'cak', 'clontong'],
  category: 'games',
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'clontong') {
      const game = sock.caklontong[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}caklontong*.`)
      ...
      const clue = game.soal.jawaban.replace(/[AIUEOaiueo]/g, '_')
...
Ketik *${prefix}clontong* untuk melihat petunjuk.
Hadiah: ${reward} Money`.trim()
    ...
    const timer = setTimeout(() => {
      if (sock.caklontong?.[m.chat]) {
        sock.sendMessage(m.chat, { text: `Waktu habis. Jawabannya adalah *${soal.jawaban}*.\n_${soal.deskripsi}_` }, { quoted: sent }).catch(() => {})
        delete sock.caklontong[m.chat]
      }
    }, TIMEOUT_MS)

    sock.caklontong[m.chat] = { key: sent.key, soal, reward, timer }
  },
