...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/asahotak.json'
...
function endGame(sock, chat) {
  const game = sock.asahotak?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.asahotak[chat]
}

export default {
  cmd: ['asahotak', 'otak', 'aotak'],
  category: 'games',
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'aotak') {
      const game = sock.asahotak[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}asahotak*.`)
...
Ketik *${prefix}aotak* untuk melihat petunjuk.
Hadiah: ${reward} Money`.trim()
    ...
    sock.asahotak[m.chat] = { key: sent.key, soal, reward, timer }
  },
