...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/siapakahaku.json'
...
function endGame(sock, chat) {
  const game = sock.siapakahaku?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.siapakahaku[chat]
}

export default {
  cmd: ['siapakahaku', 'siapa'],
  category: 'games',
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'siapa') {
      const game = sock.siapakahaku[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}siapakahaku*.`)
...
Hadiah: ${reward} Money`.trim()
    ...
    sock.siapakahaku[m.chat] = { key: sent.key, soal, reward, timer }
  },
