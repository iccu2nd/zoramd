 ...
 *
 * Idle-latency fix:
 * - Long TTL so first command after idle does not always hit Mongo.
 * - Singleflight per key so a burst of messages after idle does not stampede Mongo.
 * - API path must call invalidateFeatureCache when settings change.
 */
...
const TTL = Number(process.env.FEATURE_CACHE_TTL_MS || 15 * 60_000)

const cmdMapCache = new Map() // key: botId -> { data: Map(customCmdLower -> featureKey), expires }
const CMDMAP_TTL = Number(process.env.FEATURE_CMDMAP_TTL_MS || 15 * 60_000)

const inflight = new Map()
const cmdMapInflight = new Map()
...
export function invalidateFeatureCache(botId, featureKey) {
    ...
    for (const k of [...inflight.keys()]) {
        if (k.startsWith(botId + ':')) inflight.delete(k)
    }
    cmdMapInflight.delete(botId)
}
...
export async function getCustomCommandMap(botId) {
    ...
    let pending = cmdMapInflight.get(botId)
    if (!pending) {
        pending = (async () => {
            const map = new Map()
            try {
        ...
        cmdMapInflight.set(botId, pending)
    }
    return pending
}
...
export function checkAccessRule(rule, m) {
    ...
    const isPrem = !!(user && (user.premium === true || (typeof user.premiumTime === 'number' && user.premiumTime > Date.now())))
    const isVerified = !!(user && user.registered)

    return rules.some(r => {
        if (r === 'owner') return !!m.isOwner
        if (r === 'admin') return !!m.isAdmin
        if (r === 'premium') return isPrem || !!m.isOwner
        if (r === 'verified') return isVerified || !!m.isOwner
