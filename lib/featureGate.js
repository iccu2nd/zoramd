...
const TTL = Number(process.env.FEATURE_CACHE_TTL_MS || 10 * 60_000)

const cmdMapCache = new Map() // key: botId -> { data: Map(customCmdLower -> featureKey), expires }
const CMDMAP_TTL = Number(process.env.FEATURE_CMDMAP_TTL_MS || 10 * 60_000)
...
export async function resolveFeature(botId, featureKey) {
    ...
    try {
        const data = await getCached(botId, featureKey)
        return {
            ...fallback,
            ...data,
            premiumOnly: !!(data && data.premiumOnly)
        }
    } catch {
        return fallback
    }
