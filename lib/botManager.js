...
class BotInstance {
    ...
    async start({ phoneNumber, forcePairing = false, clearSessionFirst = false } = {}) {
        ...
        // Jangan hapus pairingCode jika ini restart setelah 515
        const keepPairing = this.pairingCode
        this.qrDataUrl = null
        this.lastError = null
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                // Simpan state pairing sebelum reset socket
                const savedPairingCode = this.pairingCode
                const wasPairing = this._wantPairing || this._pairingInProgress || this.status === 'pairing'
                const wasQr = this.status === 'qr' || (!this._wantPairing && this.status === 'connecting')
                ...
                // 515 / Stream Errored = NORMAL setelah requestPairingCode.
                // WA minta restart socket DENGAN session yang sama (jangan clear, jangan hapus pairing code).
                const restartRequired =
                    statusCode === DisconnectReason.restartRequired ||
                    statusCode === 515 ||
                    /restart required/i.test(String(errMsg)) ||
                    /Stream Errored/i.test(String(errMsg))

                console.log(chalk.yellowBright(`[${this.sessionId}] Close: ${errMsg} (${statusCode})`))

                if (restartRequired) {
                    // Pertahankan pairing code di UI
                    if (savedPairingCode) {
                        this.pairingCode = savedPairingCode
                        this.status = 'pairing'
                        this.lastError = null
                    ...
                    // Restart socket dengan session yang SAMA (clearSessionFirst: false)
                    // Batasi supaya tidak infinite loop
                    if (this._restartCount < 5) {
                        this._restartCount++
                        clearTimeout(this._reconnectTimer)
                        this._reconnectTimer = setTimeout(() => {
                            if (this._stopped) return
                            console.log(chalk.cyanBright(`[${this.sessionId}] Restart setelah 515 (attempt ${this._restartCount})`))
                            this.start({
                                phoneNumber: undefined,
                                forcePairing: false,
                                clearSessionFirst: false
                            }).catch(e =>
                                console.error(chalk.redBright(`[${this.sessionId}] restart fail:`), e.message)
                            )
                        }, 1500)
                    } else {
                        this.lastError = 'Gagal reconnect setelah restart. Coba Hubungkan lagi.'
                        this.status = 'disconnected'
                        await setBotStatus(this.sessionId, 'disconnected').catch(() => {})
                    }
                    return
                ...
                if (!wasPairing) this.pairingCode = null
                await setBotStatus(this.sessionId, 'disconnected').catch(() => {})

                // Reconnect lembut hanya jika sudah pernah registered
                clearTimeout(this._reconnectTimer)
                this._reconnectTimer = setTimeout(() => {
                    if (!this._stopped) {
                        this.start({ clearSessionFirst: false }).catch(() => {})
                    }
                }, 5000)
            }
        })
