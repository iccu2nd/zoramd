...
import { getAccountPremiumState, applyPremiumStateToSock } from './db/subscription.js'
import { getCustomCommandMap } from './featureGate.js'
import { applyRuntimeSettingsToSock } from './botRuntimeSettings.js'
    ...
    async start(opts = {}) {
        ...
        try {
            ...
            const sock = makeWASocket({
                ...
                // Keep WA socket warm across idle gaps so first message is not delayed by reconnect.
                keepAliveIntervalMs: 25000,
                // Prefer in-memory group metadata (stale-while-revalidate in simple.js).
            ...
            sock.botConfig = cfg
            // Warm premium, runtime settings, feature map (non-blocking for first commands).
            ;(async () => {
                try {
                    await applyRuntimeSettingsToSock(sock, this.sessionId)
                } catch {}
                try {
                    ...
                    if (ownerId) {
                        ...
                        cfg.isPremiumAccount = state.isPremium
                    }
                } catch {}
                try {
                    await getCustomCommandMap(this.sessionId)
                } catch {}
            })()
