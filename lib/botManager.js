...
import { getBotSettings, applySettingsToSock } from './botSettingsService.js'
...
class BotInstance {
    ...
    buildConfig() {
        ...
        return {
            ...
            sourceUrl: identity.sourceUrl || configBase.sourceUrl,
            isJadibot: true
        }
    }
    ...
    async start(opts = {}) {
        ...
        try {
            ...
            sock.botConfig = cfg
            // Load bot settings into sock (non-blocking)
            getBotSettings(this.sessionId).then(s => applySettingsToSock(sock, s)).catch(() => {})
