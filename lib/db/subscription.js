...
export async function getAccountSubscription(accountId) {
    const cached = readCache(accountCache, accountId)
    if (cached) return cached
    const db = await getMongoDb()
    const sub = await db.collection(COLLECTIONS.SUBSCRIPTIONS).findOne({ accountId: String(accountId) })
    return writeCache(accountCache, accountId, sub || { accountId: String(accountId), plan: 'free', status: 'active', expiresAt: null })
}
...
export async function getSubscription(botId) {
    const cached = readCache(botCache, botId)
    if (cached) return cached
    const db = await getMongoDb()
    const sub = await db.collection(COLLECTIONS.SUBSCRIPTIONS).findOne({ botId })
    return writeCache(botCache, botId, sub || { botId, plan: 'free', status: 'active', expiresAt: null })
}
