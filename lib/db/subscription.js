...
export async function getAccountPremiumMap(accountIds) {
    const ids = [...new Set((accountIds || []).map(id => String(id)).filter(Boolean))]
    if (!ids.length) return new Map()
    const db = await getMongoDb()
    const docs = await db.collection(COLLECTIONS.SUBSCRIPTIONS)
        .find({ accountId: { $in: ids } }, { projection: { accountId: 1, plan: 1, status: 1, expiresAt: 1 } })
        .toArray()
    const now = Date.now()
    const map = new Map()
    for (const id of ids) map.set(id, false)
    for (const sub of docs) {
        const active = sub.plan === 'premium' &&
            sub.status === 'active' &&
            (!sub.expiresAt || new Date(sub.expiresAt).getTime() >= now)
        map.set(String(sub.accountId), active)
    }
    return map
}
