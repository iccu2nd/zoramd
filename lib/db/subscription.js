...
const PAID_PLANS = new Set(['premium', 'basic', 'pro', 'ultimate'])
...
export async function getAccountPlan(accountId) {
    const sub = await getAccountSubscription(accountId)
    if (!PAID_PLANS.has(sub.plan) || sub.status !== 'active') return 'free'
    if (sub.expiresAt && new Date(sub.expiresAt).getTime() < Date.now()) return 'free'
    return sub.plan === 'premium' ? 'pro' : sub.plan
}
...
export async function isBotPremium(botId) {
    ...
    if (!PAID_PLANS.has(sub.plan) && sub.plan !== 'premium') return false
    if (sub.status !== 'active') return false
    if (sub.expiresAt && new Date(sub.expiresAt).getTime() < Date.now()) return false
    return true
}
...
export async function activateAccountPremium(accountId, { months = 1, days, tier = 'pro' } = {}) {
    const db = await getMongoDb()
    const aid = String(accountId)
    const planName = PAID_PLANS.has(tier) ? (tier === 'premium' ? 'pro' : tier) : 'pro'
    const current = await getAccountSubscription(aid)
    const base = (PAID_PLANS.has(current.plan) && current.expiresAt && new Date(current.expiresAt) > new Date())
        ? new Date(current.expiresAt)
        : new Date()
    const durationMs = days ? days * 24 * 60 * 60 * 1000 : months * 30 * 24 * 60 * 60 * 1000
    const expiresAt = new Date(base.getTime() + durationMs)
    ...
    await db.collection(COLLECTIONS.SUBSCRIPTIONS).updateOne(
        { accountId: aid },
        { $set: { accountId: aid, plan: planName, status: 'active', expiresAt, updatedAt: new Date() } },
        { upsert: true }
    )
    ...
    for (const b of bots) {
        ...
        await db.collection(COLLECTIONS.SUBSCRIPTIONS).updateOne(
            ...
            { $set: { expiresAt, plan: planName, status: 'active', updatedAt: new Date() } },
            { upsert: true }
        )
        botCache.delete(String(b._id))
    }

    return { accountId: aid, plan: planName, status: 'active', expiresAt }
}
