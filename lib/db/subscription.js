...
/**
 * Full premium snapshot for socket cache.
 * When premium ends, expiresAt is in the past → isPremium false (lane free).
 */
export async function getAccountPremiumState(accountId) {
    const sub = await getAccountSubscription(accountId)
    const expiresAtMs = sub?.expiresAt ? new Date(sub.expiresAt).getTime() : null
    const isPremium =
        sub?.plan === 'premium' &&
        sub?.status === 'active' &&
...
/** Apply plan to sock so command path can demote to free the moment expiresAt passes. */
export function applyPremiumStateToSock(sock, state) {
    if (!sock || !state) return
    const isPremium = !!state.isPremium
    sock.isPremiumAccount = isPremium
    sock._premiumExpiresAt = state.expiresAtMs || null
    sock._premiumCheckedAt = Date.now()
    if (sock.botConfig) sock.botConfig.isPremiumAccount = isPremium
}

/**
 * Fast local read: if expiresAt already passed, force free without DB.
 * Returns { isPremium, needsRefresh } — needsRefresh when cache empty/stale for background update.
 */
export function readSockPremium(sock, maxAgeMs = 5 * 60_000) {
    if (!sock) return { isPremium: false, needsRefresh: true }
    const now = Date.now()
    if (sock._premiumExpiresAt && now >= sock._premiumExpiresAt) {
        sock.isPremiumAccount = false
        if (sock.botConfig) sock.botConfig.isPremiumAccount = false
        return { isPremium: false, needsRefresh: true }
    }
    const age = now - (sock._premiumCheckedAt || 0)
    const hasCache = sock._premiumCheckedAt != null
    const isPremium = !!(sock.isPremiumAccount || sock.botConfig?.isPremiumAccount)
    return {
        isPremium: hasCache ? isPremium : false,
        needsRefresh: !hasCache || age > maxAgeMs
    }
}
