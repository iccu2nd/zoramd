...
export async function deleteAccountById(accountId) {
    ...
    const account = await db.collection(COLLECTIONS.ACCOUNTS).findOne({ _id: oid })
    if (!account) return false
    // Delete owned bots first
    const bots = await db.collection(COLLECTIONS.BOTS).find({ ownerId: oid }).toArray()
    for (const bot of bots) {
        await deleteBotById(bot._id.toString())
    }
    await db.collection(COLLECTIONS.SUBSCRIPTIONS).deleteMany({ accountId: accountId.toString() })
    await db.collection(COLLECTIONS.ORDERS).deleteMany({ accountId: accountId.toString() })
    await db.collection(COLLECTIONS.ACCOUNTS).deleteOne({ _id: oid })
    return true
}
