...
// Soft TTL: serve stale metadata immediately, refresh in background so group
// messages after idle never block the command path on sock.groupMetadata().
...
async function refreshGroupMetadata(sock, jid) {
    ...
    try {
        ...
        // Keep stale entry; next message can retry.
    } finally {
...
/**
 * Prefer cached metadata. On soft-TTL expiry, return stale value immediately
 * and refresh in background. Only block on a true cache miss (first sighting).
 */
export async function getCachedGroupMetadata(sock, jid) {
    ...
    if (cached) {
        ...
        if (Date.now() - fetchedAt > GROUP_META_SOFT_TTL_MS) {
            // Stale-while-revalidate: do not block the message path.
            setImmediate(() => { refreshGroupMetadata(sock, jid).catch(() => {}) })
        }
        return cached
    }
    // Cold miss: must fetch once. Subsequent messages for this group stay non-blocking.
    try {
        const meta = await sock.groupMetadata(jid)
        if (meta) storeGroupMeta(jid, meta)
        return meta
