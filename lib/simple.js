...
import { resolveBotConfig, stickerIdentity } from './botConfig.js'
...
function writeBounded(cache, jid, value) {
    if (cache.size >= GROUP_CACHE_MAX && !cache.has(jid)) {
        const first = cache.keys().next().value
        if (first) cache.delete(first)
    }
    cache.set(jid, { value, expiresAt: Date.now() + GROUP_CACHE_TTL_MS })
}
...
export async function getCachedGroupMetadata(sock, jid) {
    const cached = readFresh(groupMetadataCache, jid)
    if (cached) return cached
    const meta = await sock.groupMetadata(jid)
    writeBounded(groupMetadataCache, jid, meta)
    writeBounded(groupParticipantIndexCache, jid, buildParticipantIndex(meta))
    return meta
}
...
export function getCachedParticipantIndex(jid) {
    return readFresh(groupParticipantIndexCache, jid) || new Map()
}

export function getCachedGroupSubject(jid) {
    return readFresh(groupMetadataCache, jid)?.subject
}
...
export default async function wrapSocket(sock) {
    ...
    sock.sendMessage = async (jid, content, options = {}) => {
        ...
        if (result) {
            setImmediate(() => printOutgoingLog(jid, content, {
                botName: (sock.botConfig || config).botName || 'ZoraBot',
                groupName: typeof jid === 'string' && jid.endsWith('@g.us') ? getCachedGroupSubject(jid) : undefined,
                pluginName
            }))
        }
        return result
    ...
    sock.sendSticker = async (jid, buffer, quoted = '', options = {}) => {
        const id = stickerIdentity(sock, config)
        const packname = options.packname !== undefined ? options.packname : id.packname
        const author = options.author !== undefined ? options.author : id.author
        const isVideo = options.isAnimated || false

        const injectExif = async (webpBuf) => {
            const img = new WebP.Image()
            await img.load(webpBuf)
            const json = { 'sticker-pack-id': `zorabot-${Date.now()}`, 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'emojis': ['🚀'] }
