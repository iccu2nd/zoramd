...
/** Cap number of tracked sessions to avoid unbounded growth */
export function pruneSessionMetrics(maxSessions = 500) {
    if (bySession.size <= maxSessions) return
    const entries = [...bySession.entries()].sort((a, b) => (a[1].startedAt || 0) - (b[1].startedAt || 0))
    const drop = entries.length - maxSessions
