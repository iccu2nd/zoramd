...
// Jangan akses global.db di top-level — database.js init async (Mongo),
// module ini bisa di-load paralel sebelum global.db siap.
function getGd() {
    return global.db?.data || { users: {}, chats: {}, contacts: {}, lid_mapping: {}, msgs: {} }
}
...
function isMemberOfRequiredGroup(sock, jid) {
    const cfg = resolveBotConfig(sock)
    const groupId = cfg.groupId
    if (!groupId) return Promise.resolve(false)
    return getCachedGroupMetadata(sock, groupId)
        .then(meta => {
...
export async function notifyGconlyOnce(sock, m) {
    ...
    const channelUrl = cfg.channelUrl || ''
    ...
    try {
        const buttons = []
        if (groupUrl) buttons.push({ type: 'url', label: '👥 Join Grup', url: groupUrl })
        if (channelUrl) buttons.push({ type: 'url', label: '📢 Follow Saluran', url: channelUrl })
        await sock.sendInteractiveButton(m.from, {
            body: text,
            footer: cfg.botName || 'Akses private chat dibatasi',
            buttons
        })
    } catch (e) {
