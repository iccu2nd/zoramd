...
// Async (fs.promises) on purpose: this file is shared between the WA bot event
// loop and the HTTP webhook route (server/routes/api.js). A *Sync fs call blocks
// the whole process for its duration -- even a few ms of disk I/O here would
// delay every in-flight WhatsApp command, not just the donation command itself.
async function loadDB() {
    try { return JSON.parse(await fs.promises.readFile(config.dbFile, 'utf-8')) } catch { return [] }
}

async function saveDB(db) {
    await fs.promises.writeFile(config.dbFile, JSON.stringify(db, null, 2))
}
...
export async function updateStatus(orderId, status, data) {
    const db = await loadDB()
