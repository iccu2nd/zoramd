...
import { pushChatLog } from './liveChatlog.js'
...
export function printChatLog(m, sessionId) {
    const payload = {
        isGroup: m.isGroup,
    ...
    setImmediate(() => printLog(payload))
    try { pushChatLog(sessionId, payload) } catch (_) {}
}
...
export function printOutgoingLog(jid, content, opts = {}) {
    try {
        const { botName = 'Bot', groupName, pluginName, sessionId } = opts
        const payload = {
