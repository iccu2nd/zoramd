...
import { getAccountPremiumMap } from './db/subscription.js'
...
async function tick() {
    try {
        ...
        const premiumByAccount = await getAccountPremiumMap(
            bots.map(bot => bot.ownerId?.toString?.() || String(bot.ownerId))
        )
        ...
        for (const bot of bots) {
            try {
                ...
                if (premiumByAccount.get(ownerId)) continue
...
export async function sendAdsManually({ target, text, skipPremium }) {
    ...
    const results = []
    const premiumByAccount = skipPremium
        ? await getAccountPremiumMap(bots.map(item => item.ownerId?.toString?.() || String(item.ownerId)))
        : new Map()
    ...
    for (const bot of bots) {
        try {
            if (skipPremium) {
                const ownerId = bot.ownerId?.toString?.() || String(bot.ownerId)
                if (premiumByAccount.get(ownerId)) {
