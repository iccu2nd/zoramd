...
export async function serialize(sock, m) {
    ...
    // Prefer per-bot live settings (sock._botSettings), fallback to legacy module settings
    const extraOwners = (sock._botSettings?.extraOwners || settings.extraOwners || [])
    m.isOwner = ownerNumbers.includes(m.sender) || extraOwners.includes(m.sender)
