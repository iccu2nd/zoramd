...
let botDataCol = null
let botSettingsCol = null
let dataDoc = null
let settingsDoc = null
if (process.env.MONGODB_URI) {
    try {
        const mongo = await getMongoDb()
        botDataCol = mongo.collection(COLLECTIONS.BOT_DATA)
        botSettingsCol = mongo.collection(COLLECTIONS.BOT_SETTINGS)
        ;[dataDoc, settingsDoc] = await Promise.all([
            botDataCol.findOne({ botId }),
            botSettingsCol.findOne({ botId })
        ])
    } catch (e) {
        console.warn('Database MongoDB tidak tersedia; memakai runtime memory:', e.message)
    }
}
...
const flushTimer = setInterval(async () => {
    if (!botDataCol || !botSettingsCol) return
    try {
        await Promise.all([
