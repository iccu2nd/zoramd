...
function quickHash(obj) {
    // Hash murah buat deteksi "ada yang berubah?" tanpa nulis ke Mongo tiap tick
    // kalau isinya sama persis dengan flush terakhir (bot idle / gak ada aktivitas).
    return crypto.createHash('sha1').update(JSON.stringify(obj)).digest('hex')
}

// Placeholder sync supaya module lain (gconly, plugins) tidak crash
// kalau di-load paralel sebelum await Mongo selesai.
if (!global.db) {
    global.db = {
        data: {
            users: {},
...
let lastDataHash = quickHash(gd)
let lastSettingsHash = quickHash(settings)
...
setInterval(async () => {
    try {
        const dataHash = quickHash(gd)
        const settingsHash = quickHash(settings)
        const jobs = []
        ...
        if (settingsHash !== lastSettingsHash) {
            jobs.push(
                botSettingsCol.updateOne(
                    ...
                    { upsert: true }
                ).then(() => { lastSettingsHash = settingsHash })
            )
        }
