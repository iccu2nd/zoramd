...
const DATA_URL = 'https://raw.githubusercontent.com/reyzdesu/bot-assets/main/games/tebakkata.json'
...
function endGame(sock, chat) {
  const game = sock.tebakkata?.[chat]
  if (!game) return
  clearTimeout(game.timer)
  delete sock.tebakkata[chat]
}
...
export default {
  ...
  run: async (m, { sock, prefix, cmd }) => {
    ...
    if (cmd === 'tkata') {
      const game = sock.tebakkata[m.chat]
      if (!game) return m.reply(`Tidak ada soal yang sedang berlangsung di sini. Mulai dulu dengan *${prefix}tebakkata*.`)
...
Hadiah: ${reward} Money`.trim()
    ...
    sock.tebakkata[m.chat] = { key: sent.key, soal, reward, timer }
  },
